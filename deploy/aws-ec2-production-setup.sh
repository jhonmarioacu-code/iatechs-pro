...
################################################################################
# IAtechs Pro - AWS EC2 Production/Staging Installer
#
# Target:
...
require_root() {
    if [[ "${EUID}" -ne 0 ]]; then
        fail "Run this script as root: sudo bash deploy/aws-ec2-production-setup.sh"
    fi
}
...
install_nginx() {
    ...
    local server_name="_ 127.0.0.1 localhost"
    if [[ -n "${DOMAIN}" ]]; then
        server_name="${DOMAIN} _ 127.0.0.1 localhost"
    fi
...
configure_laravel() {
    ...
    if [[ "${RUN_SEEDERS}" == "yes" ]]; then
        run_app php artisan db:seed --force
    fi
